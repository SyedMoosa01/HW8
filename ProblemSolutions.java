...
class ProblemSolutions {
    ...
    public boolean canFinish(int numExams,
                             int[][] prerequisites) {
        // Converting prerequisites into a map where key points to its prerequisite
        HashMap<Integer, Integer> map = getPrerequisiteMap(prerequisites);

        // Check for cycles for each exam
        for (int i = 0; i < numExams; i++) {
            HashSet<Integer> visited = new HashSet<>();
            int current = i;

            // Traversing through the map for the current node
    ...
    // Helper method to convert prerequisites into a HashMap
    private HashMap<Integer, Integer> getPrerequisiteMap(int[][] prerequisites) {
        HashMap<Integer, Integer> map = new HashMap<>();
        for (int[] prerequisite : prerequisites) {
            map.put(prerequisite[0], prerequisite[1]);
        }
        return map;
    ...
    public int numGroups(int[][] adjMatrix) {
        ...
        // Explore each node to count connected components
        for (int i = 0; i < numNodes; i++) {
            if (!visited[i]) {
                groupCount++;
                dfs(i, graph, visited); // Perform DFS to mark all nodes in the group
            }
        }
